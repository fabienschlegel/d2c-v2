export const SITE_NAME = 'Dévoreur 2 Code';

export const SITE_DESCRIPTION: { [key: string]: string } = {
  en: 'Another blog from a developer',
  fr: 'Un autre blog de développeur',
};

export const DEFAULT_LOCALE = 'en';

export const SITE_URL = 'https://www.devoreur2code.com';

export const SITE_TWITTER_HANDLE = 'devoreur2code';

export const SITE_IMAGE = '/assets/media/D2C-fond-blanc.webp';

export const FAVICON_URL = '/assets/media/Logo-D2C-fond-blc.png';

export const TWITTER_URL = 'https://twitter.com/fabienschlegel';

export const LINKEDIN_URL = 'https://www.linkedin.com/in/fabien-schlegel/';

export const GITHUB_PROFILE_URL = 'https://github.com/fabienschlegel';

export const WORDS_PER_MINUTE = 225;

export const RSS_FEED_URL = '/rss/feed';
export const RSS_FEED_EXT = '.xml';

export const JSON_FEED_URL = '/rss/feed';
export const JSON_FEED_EXT = '.json';

export const ATOM_FEED_URL = '/rss/atom';
export const ATOM_FEED_EXT = '.xml';
